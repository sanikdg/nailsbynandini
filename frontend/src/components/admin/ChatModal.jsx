...
export const ChatModal = ({ booking, onClose, adminName }) => {
  const { user } = useAuth();
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      // Extract target user ID (the customer, not the admin)
      const targetUserId = typeof booking.userId === 'object'
        ? booking.userId._id
        : booking.userId;
                  ...
                  >
                    {!isOwnMessage && (
                      <p className="text-xs font-semibold mb-1 opacity-70">
                        {typeof msg.senderId === 'object' ? msg.senderId.name : 'Customer'}
                      </p>
                    )}
