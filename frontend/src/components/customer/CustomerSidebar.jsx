...
import { Palette, Calendar, Sparkles, ChevronLeft, ChevronRight, Home, LogOut, Star } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../../context/AuthContext';

export const CustomerSidebar = ({ activeTab, setActiveTab }) => {
  const navigate = useNavigate();
  const { logout } = useAuth();
  const [isCollapsed, setIsCollapsed] = useState(false);

  const tabs = [
    { id: 'book', label: 'Book Design', icon: Palette },
    { id: 'my-appointments', label: 'My Appointments', icon: Calendar },
    { id: 'custom-request', label: 'Request Design', icon: Sparkles },
  ...
  const handleTabChange = (tabId) => {
    ...
    } else if (tabId === 'my-appointments') {
      navigate('/dashboard/my-appointments');
    } else if (tabId === 'custom-request') {
      navigate('/dashboard/custom-request');
